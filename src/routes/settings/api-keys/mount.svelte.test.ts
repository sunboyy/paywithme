...
// Rendered-markup tests for `/settings/api-keys` (PLAN §16.8). Covers the two
// acceptance criteria that are only visible in the DOM: the first-run empty
// state offering BOTH a create CTA and the API docs, and a key row showing every
// field (name, scope badge, `start` prefix, created, last-used, expiry) with a
// REAL revoke form — no collapsing on mobile, no JS-only controls.
...
describe('/settings/api-keys', () => {
	...
	it('keeps the settings tabs reachable, with API keys marked current', () => {
		const { getByRole } = render(Page, { props: { data: pageData([]) } });

		const nav = getByRole('navigation', { name: 'Settings sections' });
		const current = nav.querySelector('[aria-current="page"]');
		expect(current?.textContent?.trim()).toBe('API keys');
		expect(current?.getAttribute('href')).toBe('/settings/api-keys');
	});
});
