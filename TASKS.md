# TASKS — Pay with me autonomous build

Single source of truth for build progress. Only the **orchestrator** edits this.
Decomposed from `PLAN.md` §14. Work top-to-bottom within a phase; respect `deps`.
...
- [ ] 1.6 `.env.example` documenting every env var (Neon pooled+direct, MAILGUN_*, EMAIL_FROM, rpID/origin/trustedOrigins) — PLAN #24  @todo  deps:1.5
...
- [ ] 2.3 `lib/server` email helper (Mailgun HTTP API) with **local console-log fallback** — PLAN #24. NEEDS-INPUT: real MAILGUN_* for live send (build local path now)  @todo  deps:2.1
