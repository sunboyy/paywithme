...
flow every directory connector uses. The one place headers are genuinely _better_
...
reduce to _validate a credential → resolve a user → run the tool_.
...
_"Couldn't reach the MCP server."_
