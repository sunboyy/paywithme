...
`Idempotency-Key` header** — `src/lib/server/api/create.ts`: *"Header absent → the
create runs directly (the current at-least-once behavior)."*
...
> Agent calls `create_transaction`. The response is slow, or errors *after* the
> write committed, or the agent misreads the result.
> Agent: *"That didn't seem to go through, let me try again."*
> Two ฿240 lunches.
...
The design must distinguish *the same intent, retried* from *the same expense,
twice*. Time is the only signal available.
...
**The window must slide, not bucket.** A naive `floor(now / 60s)` is a *bucket*:
...
A replay is surfaced in the echo-back, not hidden: *"That transaction was already
recorded 3s ago — not duplicating it."*
...
- A user who *intends* to record two identical transactions within 60 seconds will
...
- Does **not** protect against an agent that retries with *different* arguments
